...
```{r gerar 1000 diferenças nas médias das notas via aleatorização, echo=FALSE}
diff_mean_lp5ef <- saeb.2021.5EF %>%
  select(PROFICIENCIA_LP_SAEB, Sexo) %>%
  filter(Sexo %in% c('Masculino', 'Feminino') & !is.na(PROFICIENCIA_LP_SAEB)) %>%
  specify(PROFICIENCIA_LP_SAEB ~ Sexo) %>%
  hypothesize(null = "independence") %>%
  generate(reps = 1000, type = "permute") %>%
  calculate(stat = "diff in means", order = c("Masculino", "Feminino"))

head(diff_meanlp5ef, 6)
```
...
```
It looks like the residuals associated with the model shows a violation of the linear technical conditions. Let's see the graphical relation between the response and explanatory variables.

```{r }
saeb.2021.5EF.regressão %>% ggplot(aes(x = log(Sexo), y = log(PROFICIENCIA_LP_SAEB))) + geom_point()
```   