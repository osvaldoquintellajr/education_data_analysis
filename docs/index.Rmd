...
censo.escolar %>% 
  ...
  geom_ribbon(aes(ymax = Proporção_escolas_acesso_internet, ymin = 0,  fill = Localização), alpha =0.2) + 
  theme_wsj() + scale_x_continuous(limits = c(2010, 2021), breaks = seq(2010,2022,by =1)) +
...
censo.escolar %>% na.omit() %>%
  group_by(Dependência_Administrativa) %>% summarise('Quantidade_tablets_disponíveis' = sum(Quantidade_computadores_em_uso_pelos_alunos_desktop))
```
