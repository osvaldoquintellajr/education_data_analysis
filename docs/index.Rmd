...
#### **Distribution of student's achievement.**

The distribution of student's achievement in portuguese by sex. 
The results are from students of the fifth year of elementary school. 

```{r distribuição das notas de meninos e meninas do 5EF em português, echo=FALSE, warning=FALSE}
saeb.2021.5EF %>%
  filter(Sexo %in% c("Masculino", "Feminino")) %>%
  ggplot(aes(x = PROFICIENCIA_LP_SAEB, color = Sexo)) + 
  geom_density() +
  theme_wsj() +
  ggtitle("Distribuição da nota em língua portuguesa por sexo_5EF") +
  theme(title = element_text(family = 'serif', size = 11),
        legend.title = element_blank(),
        axis.title.x = element_text(size = 11))
```

The distribution of student's achievement in math by sex. The results are from 
students of the fifth year of elementary school. 

```{r distribuição das notas de meninos e meninas do 5EF em matemática, echo=FALSE, warning=FALSE}
saeb.2021.5EF %>%
  filter(Sexo %in% c("Masculino", "Feminino")) %>%
  ggplot(aes(x = PROFICIENCIA_MT_SAEB, color = Sexo)) + 
  geom_density() +
  theme_wsj() +
  ggtitle("Distribuição da nota em matemática por sexo_5EF") +
  theme(title = element_text(family = 'serif', size = 11),
        legend.title = element_blank(),
        axis.title.x = element_text(size = 11)) +
...
```{r distribuição das notas de meninos e meninas do 9EF em português, echo=FALSE, warning=FALSE}
saeb.2021.9EF %>%
  filter(Sexo %in% c("Masculino", "Feminino")) %>%
  ggplot(aes(x = PROFICIENCIA_LP_SAEB, color = Sexo)) + 
  geom_density() +
  theme_wsj() +
  ggtitle("Distribuição da nota em língua portuguesa por sexo_9EF") +
  theme(title = element_text(family = 'serif', size = 11),
        legend.title = element_blank(),
        axis.title.x = element_text(size = 11))
```
...
```{r distribuição das notas de meninos e meninas do 9EF em matemática, echo=FALSE, warning=FALSE}
saeb.2021.9EF %>%
  filter(Sexo %in% c("Masculino", "Feminino")) %>%
  ggplot(aes(x = PROFICIENCIA_MT_SAEB, color = Sexo)) + 
  geom_density() +
  theme_wsj() +
  ggtitle("Distribuição da nota em matemática por sexo_9EF") +
  theme(title = element_text(family = 'serif', size = 11),
        legend.title = element_blank(),
        axis.title.x = element_text(size = 11))
```
...
saeb.2021.5EF %>%
  ...
  geom_boxplot() +
  theme_wsj() + 
  ggtitle("Boxplot nota em língua portuguesa_quinto ano do ensino fundamental.") +
  theme(title = element_text(family = 'serif', size = 10)) + 
...
saeb.2021.5EF %>%
  ...
  geom_boxplot() +
  theme_wsj() + 
  ggtitle("Boxplot da nota em matemática_quinto ano do ensino fundamental.") +
  theme(title = element_text(family = 'serif', size = 11)) + 
  scale_y_continuous(limits = c(0, 400))
```
...
saeb.2021.9EF %>%
  ...
  geom_boxplot() +
  theme_wsj() + 
  ggtitle("Boxplot nota em língua portuguesa_nono ano do ensino fundamental.") +
  theme(title = element_text(family = 'serif', size = 11)) + 
  scale_y_continuous(limits = c(0, 400))
```
...
saeb.2021.9EF %>%
  ...
  geom_boxplot() +
  theme_wsj() + 
  ggtitle("Boxplot nota em matemática_nono ano do ensino fundamental.") +
  theme(title = element_text(family = 'serif', size = 11)) + 
...
Is there a statistical significant difference between boy's and girl's
achievement? Let's answer this question.
The null hypotheses says there is no difference between boy's and girl's average achievement.
The alternative hypotheses says take girls has higher achievement than boys.

The result bellow are the difference between the girl's achievement and 
boy's achievement. 

```{r cálculo da diferença entre a nota em portugues das meninas e dos meninos 5EF, echo=FALSE}
saeb.2021.5EF %>%
  select(PROFICIENCIA_LP_SAEB, Sexo) %>%
  filter(Sexo %in% c('Masculino', 'Feminino')) %>%
  filter(!is.na(PROFICIENCIA_LP_SAEB)) %>%
  group_by(Sexo) %>%
  summarise(mean_achievement = mean(PROFICIENCIA_LP_SAEB))%>%
  pull() %>% diff()

diff_mean_obs <- saeb.2021.5EF %>%
  select(PROFICIENCIA_LP_SAEB, Sexo) %>%
  filter(Sexo %in% c('Masculino', 'Feminino')) %>%
  filter(!is.na(PROFICIENCIA_LP_SAEB)) %>%
  group_by(Sexo) %>%
...
- Now, let's check the summary of a linear regression. The variables are coded as follows:
  - Sexo == 1 se masculino
  - Sexo == 0 se feminino
  - Localização == 1 se urbana
  - Localização == 0 se rural
  - Raça == 1 se branca
  - Raça == 0 não branca
  - Escolaridade_Mãe == 1 se tem ensino superior
  - Escolaridade_Mãe == 0 se não tem ensino superior
  - Escolaridade_pai == 1 se tem ensino superior
  - Escolaridade_pai == 0 se não tem ensino superior.
  - Já_foi_reprovado == 1 se o aluno já foi reprovado.
  - Já_foi_reprovado == 0 se o aluno nunca foi reprovado.
  - INSE_ALUNO == Indicador do nível sócio-econômico do aluno (Variável que indica o nível sócio-econômico do aluno)


```{r regressão linear múltipla com dummy para sexo, echo=FALSE}
saeb.2021.5EF.regressão <- saeb.2021.5EF %>% 
  select(PROFICIENCIA_LP_SAEB, Sexo, Localização, Rede_de_Ensino, INSE_ALUNO, Raça,
         Escolaridade_Mãe, Escolaridade_Pai, Já_foi_reprovado) %>%
  filter(Sexo %in% c("Masculino", "Feminino")) %>%
  filter(Raça %in% c("Branca", "Preta", "Parda", "Amarela", "Indígena")) %>%
  filter(Escolaridade_Mãe %in% c("Não completou o 5° ano do Ensino Fundamental", "Ensino Fundamental, até o 5° ano", "Ensino Fundamental completo", "Ensino Médio completo", "Ensino Superior completo (faculdade ou graduação)")) %>%
  filter(Escolaridade_Pai %in% c("Não completou o 5° ano do Ensino Fundamental", "Ensino Fundamental, até o 5° ano", "Ensino Fundamental completo", "Ensino Médio completo", "Ensino Superior completo (faculdade ou graduação)")) %>%
  filter(Já_foi_reprovado %in% c('Não', 'Sim, uma vez', 'Sim, duas vezes ou mais')) %>%
  mutate(Sexo = if_else(Sexo == "Masculino", 1, 0),
         Localização = if_else(Localização == "Urbana", 1, 0),
         Raça = if_else(Raça == "Branca", 1, 0),
         Escolaridade_Mãe = if_else(Escolaridade_Mãe =='Ensino Superior completo (faculdade ou graduação)', 1, 0),
         Escolaridade_Pai = if_else(Escolaridade_Pai == 'Ensino Superior completo (faculdade ou graduação)', 1, 0),
         Já_foi_reprovado = if_else(Já_foi_reprovado %in% c('Sim, duas vezes ou mais', 'Sim, uma vez'),1,0))
  
my.model1 <- lm(PROFICIENCIA_LP_SAEB ~ Sexo + Localização + Raça + Escolaridade_Mãe +Escolaridade_Pai + Já_foi_reprovado + INSE_ALUNO, data = saeb.2021.5EF.regressão)
summary(my.model1)
```
